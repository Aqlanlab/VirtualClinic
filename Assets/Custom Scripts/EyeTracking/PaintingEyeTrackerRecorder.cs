    ...
    public class PaintingEyeTrackingRecorder : MonoBehaviour
    {
        ...
        private InputAction gazeTrackingStateAction;
    ...
    private void OnEnable()
    {
            ...
            gazeTrackingStateAction = new InputAction(
                name: "Eye Gaze Tracking State",
                type: InputActionType.PassThrough,
                binding: "<EyeGaze>/pose/trackingState");

            gazeTrackingStateAction.Enable();
    }
        ...
        int trackingState =
            gazeTrackingStateAction.ReadValue<int>();
        /*
         0 = Nothing valid
         1 = Position valid
         2 = Rotation valid
         3 = Position + Rotation valid */
        bool gazeValid =
            (trackingState & 2) != 0;   // Rotation is valid

        Quaternion eyeLocalRotation =
                gazeRotationAction.ReadValue<Quaternion>();

            // Eye gaze rotation is being used as a local rotation underneath the camera,
            // so convert it into a world-space direction.
            Quaternion gazeRotation =
        gazeRotationAction.ReadValue<Quaternion>();

            Vector3 gazeDirection =
                (gazeRotation * Vector3.forward).normalized;
            ...
            Debug.Log(
                ...
                " | Gaze Valid: " + gazeValid +
                " | Eye Rotation: " + gazeRotation.eulerAngles);

        if (gazeValid)
            {
                if (drawDebugRay)
                {
                    // GREEN = headset/head direction
                    Debug.DrawRay(
                        gazeOrigin,
        ...
        private void OnDisable()
        {
            ...
            if (gazeTrackingStateAction != null)
            {
                gazeTrackingStateAction.Disable();
                gazeTrackingStateAction.Dispose();
                gazeTrackingStateAction = null;
            }
    }
